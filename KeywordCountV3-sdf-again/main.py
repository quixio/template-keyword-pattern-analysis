...
def sum_keywords_tumbling(row: dict, state: State, some_param):
    ...
    if current_timestamp > (prev_start_dt + timedelta(minutes=window_length)):
        ...
        result.append(counts[window_start_str])
...
def sdf_way():
    ...
    sdf = sdf.apply(lambda row, state: sum_keywords_tumbling(row, state, "thing"), stateful=True, expand=True)
    #sdf = sdf[sdf not None]
